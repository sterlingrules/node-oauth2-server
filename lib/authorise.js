...
function getBearerToken (done) {
  ...
  if (methodsUsed > 1) {
    ...
    return res.json({ status: 'api_error', message: 'Only one method may be used to authenticate at a time (Auth header, GET or POST).' });
  } else if (methodsUsed === 0) {
    // return done(error('invalid_request', 'The access token was not found'));
    return res.json({ status: 'api_error', message: 'The access token was not found' });
  }
  ...
  if (headerToken) {
    ...
    if (!matches) {
      ...
      return res.json({ status: 'api_error', message: 'Malformed auth header' });
    }
  ...
  if (postToken) {
    if (this.req.method === 'GET') {
      ...
      return res.json({ status: 'api_error', message: 'Method cannot be GET When putting the token in the body.' });
    }
    ...
    if (!this.req.is('application/x-www-form-urlencoded')) {
      ...
      return res.json({ status: 'api_error', message: 'When putting the token in the body, content type must be application/x-www-form-urlencoded.' });
    }
  }
...
function checkToken (done) {
  ...
  this.model.getAccessToken(this.bearerToken, function (err, token) {
    ...
    if (!token) {
      ...
      return res.json({ status: 'api_error', message: 'The access token provided is invalid.' });
    }

    if (token.expires !== null &&
      (!token.expires || token.expires < new Date())) {
      // return done(error('invalid_token', 'The access token provided has expired.'));
      return res.json({ status: 'api_error', message: 'The access token provided has expired.' });
    }
